<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!-- 同步 -->
      <el-col :span="1.5">
      <el-button :icon="icon" class="filter-item" type="warning" @click="synchronize">同步</el-button>
      </el-col>
        <el-col :span="1.5">
        <el-button
          type="success"
    ...
    <el-dialog v-model="dialogConfig" :close-on-click-modal="false" title="七牛云配置" append-to-body width="580px">
      <el-form ref="configRef" :model="formConfig" :rules="rulesConfig" style="margin-top: 6px;" size="small" label-width="110px">
        <el-form-item label="Access Key" prop="accessKey">
          <el-input v-model="formConfig.accessKey" style="width: 95%" placeholder="accessKey，在安全中心，秘钥管理中查看" />
        </el-form-item>
        <el-form-item label="Secret Key" prop="secretKey">
          <el-input v-model="formConfig.secretKey" type="password" style="width: 95%;" placeholder="secretKey，在安全中心，秘钥管理中查看" />
        </el-form-item>
        <el-form-item label="空间名称" prop="bucket">
          <el-input v-model="formConfig.bucket" style="width: 95%;" placeholder="存储空间名称作为唯一的 Bucket 识别符" />
        </el-form-item>
        <el-form-item label="外链域名" prop="host">
          <el-input v-model="formConfig.host" style="width: 95%;" placeholder="外链域名，可自定义，需在七牛云绑定" />
        </el-form-item>
        <el-form-item label="存储区域">
          <el-select v-model="formConfig.zone" placeholder="请选择存储区域">
            <el-option
                v-for="item in zones"
                :key="item"
                :label="item"
        ...
        <el-form-item label="空间类型" prop="type">
          <el-radio v-model="formConfig.type" label="公开">公开</el-radio>
          <el-radio v-model="formConfig.type" label="私有">私有</el-radio>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="text" @click="dialogConfig = false">取消</el-button>
        <el-button :loading="loadingConfig" type="primary" @click="doSubmitConfig">确认</el-button>
      </div>
    </el-dialog>
  </div>
</template>

<script setup name="QiniuContent">
...
const {formConfig, rulesConfig} = toRefs(dataConfig)
...
function doSubmitConfig() {
  proxy.$refs['configRef'].validate((valid) => {
    if (valid) {
      ...
      updateQiniuCofig(formConfig.value).then(res => {
